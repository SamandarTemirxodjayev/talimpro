...
exports.findByLang = async (req, res) => {
	const filePath = path.join(__dirname, '../database', `${req.params.lang}-lang.json`);
  // Read the file
...
exports.createLang = async (req, res) => {  
  try {
    await updateOrAddObject(`./database/${req.params.lang}-lang.json`, req.body);
    return res.json({
      status: "success",
      data: {
...
exports.deleteObj = async (req, res)=> {
  try {
    await deleteObject(`./database/${req.params.lang}-lang.json`, req.params.name);
    return res.json({
      message: "success",
      data: {
